...
#### Tidyverse Packages
When a user installs the Tidyverse, as illustrated above, XX packages are installed. Then when the user loads the tidyverse using `library(tidyverse)` a core subset of YY packages are loaded into R. To use any of the pacakges not loaded with the core Tidyverse a user must explicitly load those packages (e.g. `library(readxl)`) or call the packages using the package name prefix (e.g. `readxl::readxl()` to run the `readxl()` function from the `readxl` package) **JD pick a different example FFS**

The tables below list which packages are in the "Core Tidyverse" and get loaded with `library(tidyverse)` and which must be called individually even though they are installed withe the Tidyverse meta-package. 
