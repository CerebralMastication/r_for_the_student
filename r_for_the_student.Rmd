...
Equiping students  with a popular tool that enables them to work quickly while learning is something we are certain Adam Smith would appreciate.
...
Typically, before building any models or doing other analysis, students benefit from learning basic exploratory data analysis (EDA). One tool for getting students a quick win with learning data visualization is to use `ggplot2` with the helper package `esquisse` to give them a graphical user interface for basic `ggplot2` code  [@esquisse]. `esquisse` supports only a subset of the myriad of features available in `ggplot2` but because it is a drag and drop GUI, it becomes a huge helper in jump starting students to see how code can take data and turn it into tangible visualizations. 
