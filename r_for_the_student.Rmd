...
<!-- I looked at this webpage and updated this previous information.  It seems a little different that what you origionally had.  Was my website newer (or older).  I just want to make sure I have the accurate stuff. -->
<!-- https://www.tidyverse.org/packages/ -->

<!-- Then i looked at this website (where I think you got your information).  My link above refrences yours.  I'm so confused.  I can change it back if you think this is more updated. -->
<!-- https://www.tidyverse.org/packages/ -->

<!-- There were 19 in each one.... -->
...
```{r showskimr, warning=FALSE, message=FALSE, results='asis'}
library(skimr)
kable(skim(gapminder))
```
...
Typically before building any models or doing other analysis, students benefit from learning to do basic exploritory data analysis (EDA). One tool for getting students a quick win with learning data visualization is to use `ggplot2` with the helper package `esquisse` to give them a graphical user interface that helps them write `ggplot2` code[@esquisse]. 
...
A supplemental technique to explore data is to look at a pairs plot.  The `GGally` package provides a powerful tool with simple syntax that provide the user with a massive amount of clearly organized information about the data[@GGallys].  Install the library with `install.packages("GGally")`.

A pairs plot does a good job of visualizing relationships between continuous variables or factors with a few levels.  In gapminder data, there are 142 different countries.  For this pairs plot, we will remove the `country` column for this visualization.
...
There is much to glean from the pairs plot.  We see that this tool provies pairwise plot and coorelations between continuous vaiabes and histograms and boxplots between discrete and continuous variables.  

Briefly, we can see that there is an increasing life expectancy and population as year increases as well as several countries that seperate themselves from the pack.  An easily createable pairs plot like this can spingboard studetns into further analysis which we'll highlight in the next section. 
...
Lastly, R's linear model function is flexible enought to easily add interaction terms.  Teaching using this syntax is a simple and easy to demonstrate.  To add an interaction term between `gdpPercap` and `year`, we add a colin between the independent variables.  
...
```{r lindia, fig.width=12, fig.height=12, message=FALSE, warning=FALSE}
library(lindia)
model2 %>%
  gg_diagnose(plot.all = TRUE, boxcox = TRUE)
```

<!-- i love how this looks almost exactly like what i did in ggplot before 'by hand' -->

```{r lindiaalt,include=FALSE, eval=FALSE}
# i prefer the previous code chunk over this one because it clumps all my plots togeter (saves space for the paper and consolidates them for the student).  Do you agree?  I'm not wed to this.
library(lindia)
model2 %>%
  gg_diagnose(plot.all = FALSE, boxcox = TRUE)
```

```{r lindiaalt2,include=FALSE, eval=FALSE}
# Just an FYI - this works too but I decided against it.  It makes the base R residual plots look ggplotty.  I don't love the base R diagnostic plots because they don't have a 'residuals vs x' plot for each 'x'.
...
From our previous critique, studetns should have all the information avilable to them to make decisions about how to improve their model.  To improve our analysis, we'll apply a transformation on `gdpPercap` to improve linearity and normality of errors.  A tranformation on the y variable is not necessary as we have no issues with constant variance and the Boxcox plot shows no value of lambda maximizes the log-liklihood of our model.  
...
Since our coeficients are still significant, we will take a look at the diagnostic plots to see if our assumptions are more plausible.  R makes it easy to repeat the same plots on the new model. 

```{r lindiadiag2, fig.width=12, fig.height=12, warning=FALSE,message=FALSE}
library(lindia)
modeladj %>%
  gg_diagnose(plot.all = TRUE, boxcox = TRUE) 
```

While there is more work to be done to validate this model, we can see that our transformation has improved the normality of erros and has improved the linearity of `gdppercap`.  

R makes this analysis easy for the student as the work is easily reproducable to other models, succint in its coding and output, and visually appealing.  
