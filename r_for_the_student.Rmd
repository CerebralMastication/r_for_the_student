...
To aide our example, we will explore the Gapminder data set.  The Gapminder dataset is created by the Gapminder Foundation which is a non-profit which promots sustainable global development.  https://www.gapminder.org
...
This likely does not satisfy the user's desire to understand the data as we still do not understand which countries are in the data among other curiosities.  We will provide more indepth methods of understanding your data in the Exploratory Data Analysis (EDA) section after we look briefly at the 6 `dplyr` verbs that allow students to begin rapidly interrogating data.   
...
Install the `parsnip` and `broom` packages, if not already installed:
